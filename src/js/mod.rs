...
/// JSLoader allows loading in scripts into the created neptune runtime
/// 
/// It supports both snapshotted and non-snapshotted runtimes
pub struct JSLoader<'a> {
    isolate: &'a mut v8::OwnedIsolate,
    global_context: v8::Global<v8::Context>
}

impl<'a> JSLoader<'a> {
    /// Creates a new JS Loader
    pub fn new(isolate: &'a mut v8::OwnedIsolate, global_context: v8::Global<v8::Context>) -> Self {
        Self { isolate, global_context }
    }

    /// Loads all files given to the JSLoader
    pub fn load(&mut self, files: Vec<(Cow<'static, str>, Cow<'static, str>)>) -> Result<(), crate::Error> {
        ...
        if let Some(value) = script.run(scope) {
            ...
            if func.call(scope, context.global(scope).into(), &[files.into(), file_order.into()]).is_none() {
                if let Some(exception) = scope.exception() {
                    let msg = exception.to_rust_string_lossy(scope);
